...
describe('maker fee (ADR-20)', () => {
    ...
    it('buyQuote reports the whole buy\'s fee — what the guarantor earns on the 1-elo stake', () => {
        // The card shows this fee under the "Поручители заработают" caption,
        // so it must be the total of the pending buy, never a per-share part.
        ...
        // the fee comes out of the 1-elo stake, so it can never reach 1
        expect(fee).toBeLessThan(1)
        // it is exactly the maker fee of the delivered share count
        expect(fee).toBeCloseTo(buyFee(q, b, 0, multiplier, c), 10)
        ...
        expect(costForShares(q, b, 0, multiplier) + fee).toBeCloseTo(1, 10)
        // the all-in per-share price stays the stake split over the shares
        expect(pricePerShare).toBeCloseTo(1 / multiplier, 10)
    })
})
