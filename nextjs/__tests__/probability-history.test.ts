...
import { mergeProbabilityHistory, type ProbabilityPoint } from '../app/markets/probabilityHistory'

function point(t: number, probabilities: Record<string, number>): ProbabilityPoint {
    return { t, probabilities }
}

describe('mergeProbabilityHistory', () => {
    it('returns history alone when there are no live points', () => {
        const history = [point(1, { a: 0.5, b: 0.5 }), point(2, { a: 0.6, b: 0.4 })]
        expect(mergeProbabilityHistory(history, [])).toEqual(history)
    })

    it('appends live points that are newer and moved some price', () => {
        const history = [point(1, { a: 0.5, b: 0.5 }), point(2, { a: 0.6, b: 0.4 })]
        const merged = mergeProbabilityHistory(history, [point(3, { a: 0.7, b: 0.3 })])
    ...
    it('keeps a live point that only moves a non-first outcome', () => {
        ...
        const merged = mergeProbabilityHistory(history, [point(2, { a: 0.5, b: 0.4, c: 0.1 })])
    ...
    it('drops the SSE connect frame echoing the current state', () => {
        ...
        const history = [point(1, { a: 0.5, b: 0.5 }), point(2, { a: 0.6, b: 0.4 })]
        const merged = mergeProbabilityHistory(history, [point(3, { a: 0.6, b: 0.4 })])
    ...
    it('drops live points a history refetch already picked up', () => {
        ...
        const merged = mergeProbabilityHistory(history, [
            point(2, { a: 0.6, b: 0.4 }),
            point(3, { a: 0.6, b: 0.4 }),
            point(5, { a: 0.8, b: 0.2 }),
        ])
    ...
    it('drops the stale connect echo once a newer bet has been refetched', () => {
        ...
        const history = [point(1, { a: 0.5, b: 0.5 }), point(4, { a: 0.7, b: 0.3 })]
        const merged = mergeProbabilityHistory(history, [point(2, { a: 0.6, b: 0.4 }), point(5, { a: 0.7, b: 0.3 })])
    ...
    it('drops consecutive duplicate live frames', () => {
        const merged = mergeProbabilityHistory(
            [point(1, { a: 0.5, b: 0.5 })],
    ...
    it('handles an empty history (market with no bets yet)', () => {
        const merged = mergeProbabilityHistory([], [point(1, { a: 0.5, b: 0.5 })])
    ...
    it('does not mutate the inputs', () => {
        ...
        mergeProbabilityHistory(history, live)
