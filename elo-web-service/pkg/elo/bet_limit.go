...
// CalcBetLimit computes the bet limit for a player.
// Formula: K * (1 / (1 + 10^((startingElo - playerElo) / D)))
// This equals the elo_pay the player would risk in a 2-player match against a starting_elo opponent.
func CalcBetLimit(playerElo, startingElo, eloConstK, eloConstD float64) float64 {
	return eloConstK / (1 + math.Pow(10, (startingElo-playerElo)/eloConstD))
}
...
func RecalculateBetLimits(ctx context.Context, q *db.Queries, playerIDs []int32) error {
	...
	settings, err := q.GetEloSettingsForDate(ctx, pgtype.Timestamptz{Time: time.Now(), Valid: true})
	if err != nil {
		return err
	}
	...
	for _, playerID := range playerIDs {
		...
		limit := CalcBetLimit(playerElo, settings.StartingElo, settings.EloConstK, settings.EloConstD)
