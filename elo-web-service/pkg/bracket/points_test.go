...
// 2. Match 1 ends with a shared top game score → 4–4–0–−2, no strict cut, the
// slot replays. Match 2: 2–4–0–−2 → cumulative 6–8–0–−4 → strict top-2
// {B, A} → completed.
func TestADRReplayExample(t *testing.T) {
	m1 := match("m1", map[string]float64{"A": 10, "B": 10, "C": 1, "D": 0})

	sts := Standings([]MatchResult{m1}, 4)
	if pointsOf(sts, "A") != 4 || pointsOf(sts, "B") != 4 || pointsOf(sts, "C") != 0 || pointsOf(sts, "D") != -2 {
		t.Fatalf("match 1 points: got %v", orderOf(sts))
	}
	if StrictCut(sts, 2) {
		t.Fatalf("4–4–0–−2 must replay (the co-leaders tie)")
	}

	m2 := match("m2", map[string]float64{"B": 10, "A": 5, "C": 1, "D": 0})
	sts = Standings([]MatchResult{m1, m2}, 4)
	if pointsOf(sts, "B") != 8 || pointsOf(sts, "A") != 6 || pointsOf(sts, "C") != 0 || pointsOf(sts, "D") != -4 {
		t.Fatalf("cumulative points: got %+v", sts)
	}
	if !StrictCut(sts, 2) {
		t.Fatalf("6–8–0–−4 must complete the slot")
	}
...
func TestTieInsidePromotedSetBlocks(t *testing.T) {
	// 8–8–0–−4 with promote 2: the cut against the rest is clean, but the two
	...
	sts := Standings([]MatchResult{m1, m2}, 4)
	if StrictCut(sts, 2) {
		t.Fatalf("8–8–0–−4 must not complete a promote-2 slot")
	}
}

func TestTieAtCutBoundaryBlocks(t *testing.T) {
	// Cumulative 4–4–4–2 with promote 2: places 2 and 3 share points at the
	// cut — the top-2 set is not strictly separated from the rest.
	m1 := match("m1", map[string]float64{"A": 10, "B": 10, "C": 1, "D": 0})
	m2 := match("m2", map[string]float64{"C": 10, "D": 10, "A": 1, "B": 1})
	sts := Standings([]MatchResult{m1, m2}, 4)
	if pointsOf(sts, "A") != 4 || pointsOf(sts, "B") != 4 || pointsOf(sts, "C") != 4 || pointsOf(sts, "D") != 2 {
		t.Fatalf("fixture drift: %+v", sts)
	}
	if StrictCut(sts, 2) {
		t.Fatalf("4–4–4–2 must not complete: the cut boundary ties")
	}
}

func TestLateOverturn(t *testing.T) {
	// A leads after match 1; match 2 flips the order — every match counts,
	// nothing is discarded. With one win apiece the points tie (2–2): the
	// 2-seat slot replays; B taking match 3 completes it 4–2.
	...
	if StrictCut(sts, 1) {
		t.Fatalf("1–1 in a 2-seat slot must replay")
	}
	if sts[0].PlayerID != mustID("B") {
		t.Fatalf("tie-break must prefer the recent winner: %+v", sts)
	}
	sts = Standings([]MatchResult{m1, m2, m3}, 2)
	if !StrictCut(sts, 1) || sts[0].PlayerID != mustID("B") || pointsOf(sts, "B") != 4 || pointsOf(sts, "A") != 2 {
